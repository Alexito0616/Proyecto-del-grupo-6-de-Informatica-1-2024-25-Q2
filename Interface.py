...
class GraphInterface:
  def __init__(self, master):
      ...
      side_panel = tk.Frame(master)
      side_panel.pack(side=tk.RIGHT, fill=tk.BOTH, padx=5, pady=5)

      tk.Label(side_panel, text="Selección del grafo").pack(pady=5)

      # Grupo de botones principales
      tk.Button(side_panel, text="Mostrar Grafo Ejemplo", command=self.show_example_graph).pack(fill=tk.X, pady=1)
      tk.Button(side_panel, text="Mostrar Grafo Inventado", command=self.show_invented_graph).pack(fill=tk.X, pady=1)
      tk.Button(side_panel, text="Cargar Grafo desde Archivo", command=self.load_graph_from_file).pack(fill=tk.X,
                                                                                                       pady=1)


      # Funcionalidades
      tk.Button(side_panel, text="Seleccionar Nodo y Ver Vecinos", command=self.select_node_mode).pack(fill=tk.X,
                                                                                                    pady=1)
      tk.Button(side_panel, text="Mostrar Alcanzables", command=self.reachability_mode).pack(fill=tk.X, pady=1)
      tk.Button(side_panel, text="Camino más corto", command=self.shortest_path_mode).pack(fill=tk.X, pady=1)


      tk.Label(side_panel, text="Modificar el grafo").pack(pady=5)


      # Edición de grafo
      tk.Button(side_panel, text="Añadir Nodo", command=self.add_node_mode).pack(fill=tk.X, pady=1)
      tk.Button(side_panel, text="Añadir Segmento", command=self.add_segment_mode).pack(fill=tk.X, pady=1)
      tk.Button(side_panel, text="Eliminar Nodo", command=self.delete_node_mode).pack(fill=tk.X, pady=1)
      tk.Button(side_panel, text="Nuevo Grafo", command=self.new_graph).pack(fill=tk.X, pady=1)
      tk.Button(side_panel, text="Guardar Grafo", command=self.save_graph_to_file).pack(fill=tk.X, pady=1)


      tk.Label(side_panel, text="Visualizar espacio aereo").pack(pady=5)


      # Botones de espacio aéreo (reubicados correctamente)
      tk.Button(side_panel, text="Espacio Aéreo Catalunya", command=self.open_airspace_gui_CAT).pack(fill=tk.X, pady=1)
      tk.Button(side_panel, text="Espacio Aéreo Espanya", command=self.open_airspace_gui_SPAIN).pack(fill=tk.X, pady=1)
      tk.Button(side_panel, text="Espacio Aéreo Europa", command=self.open_airspace_gui_EUROPE).pack(fill=tk.X, pady=1)
  ...
  def open_airspace_gui_CAT(self):
      # Crear una nueva ventana
      new_window = tk.Toplevel(self.root)


      # Cargar los datos del espacio aéreo
      airspace = AirSpace()
      airspace.load_from_files("Cat_nav.txt", "Cat_seg.txt", "Cat_aer.txt")


      # Lanzar la interfaz AirspaceGUI en esa nueva ventana
      AirspaceGUI(new_window, airspace)




  def open_airspace_gui_SPAIN(self):
      # Crear una nueva ventana
      new_window = tk.Toplevel(self.root)
  ...
  def open_airspace_gui_EUROPE(self):
      # Crear una nueva ventana
      new_window = tk.Toplevel(self.root)
      ...
      # Cargar los datos del espacio aéreo
      airspace = AirSpace()
      airspace.load_from_files("ECAC_nav.txt", "ECAC_seg.txt", "ECAC_aer.txt")
