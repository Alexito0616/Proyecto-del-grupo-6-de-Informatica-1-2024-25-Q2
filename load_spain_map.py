...
class AirspaceGUI:
   def __init__(self, master):
       self.master = master
       self.master.title("Visualizador de espacio aéreo - España")


       # Cargar el espacio aéreo de España
       self.airspace = AirSpace()
       self.airspace.load_from_files("Spain_nav.txt", "Spain_seg.txt", "Spain_aer.txt")
   ...
   def draw_graph(self, path=None, highlight_neighbors=None):
       ...
       for seg in self.airspace.navsegments:
           ...
           if path and seg.origin_id in path and seg.dest_id in path:
               ...
               if i + 1 < len(path) and path[i + 1] == seg.dest_id:
                   color = 'red'
           elif highlight_neighbors and seg.origin_id == highlight_neighbors[0] and seg.dest_id in highlight_neighbors[1]:
               color = 'orange'
       ...
       self.ax.set_title("Mapa de navegación aérea - España")
   ...
   def shortest_path(self):
       ...
       if not path:
           messagebox.showwarning("Advertencia", "No existe un camino entre los nodos seleccionados.")
       else:
